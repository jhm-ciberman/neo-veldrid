...
[Shaders](xref:NeoVeldrid.Shader) are a fundamental device resource in NeoVeldrid. Each Shader object represents a single shader module created from specialized shader code. Shader objects are combined into shader sets which are one input into a [Pipeline](xref:NeoVeldrid.Pipeline). Shaders can read from and write to several other kinds of device resources while executing.

## Creating a Shader

A [ShaderDescription](xref:NeoVeldrid.ShaderDescription) takes two pieces of information. The first piece is the [stage](xref:NeoVeldrid.ShaderStages) the shader is applicable to. The second piece of information is an API-specific byte array containing the shader code itself. The contents of this byte array depend on the specific graphics API being used (see [GraphicsDevice.BackendType](xref:NeoVeldrid.GraphicsDevice#NeoVeldrid_GraphicsDevice_BackendType)).

* Direct3D11: [ShaderBytes](xref:NeoVeldrid.ShaderDescription#NeoVeldrid_ShaderDescription_ShaderBytes) must contain HLSL bytecode or ASCII-encoded HLSL text (Shader Model 5).
...
Shader objects have a unique relationship with [ResourceLayouts](xref:NeoVeldrid.ResourceLayout) and [ResourceSets](xref:NeoVeldrid.ResourceSet). When creating a Pipeline, the provided [ResourceLayouts](xref:NeoVeldrid.GraphicsPipelineDescription#NeoVeldrid_GraphicsPipelineDescription_ResourceLayouts) must match the actual resource types that are specified in the shader code.

ResourceLayouts merely define the layout of resources expected by a set of shaders. When draw commands are executed, the actual shader resources (DeviceBuffers, TextureViews, and Samplers) are determined based on the [CommandList's](xref:command-lists) currently-bound [ResourceSets](xref:NeoVeldrid.ResourceSet).

Each ResourceLayout contains a set of one-or-more resource elements. Additionally, multiple ResourceLayout objects can be used to define the inputs of a shader set. When this is done, each ResourceLayout must be matched by a corresponding ResourceSet containing the appropriate resource types. ResourceSet objects are bound to a particular slot on a [CommandList](xref:command-lists). The slot corresponds to the index of the matching ResourceLayout given in the [ResourceLayouts](xref:NeoVeldrid.GraphicsPipelineDescription#NeoVeldrid_GraphicsPipelineDescription_ResourceLayouts) array of the [GraphicsPipelineDescription](xref:NeoVeldrid.GraphicsPipelineDescription) or [ComputePipelineDescription](xref:NeoVeldrid.ComputePipelineDescription).
...
A structured buffer is another kind of DeviceBuffer resource available to shaders. Like uniform buffers, they can be used to store arbitrary data, but are generally much larger. Structured buffers are used to store a large number of a single kind of value (a "structure"). The size of the structure that is stored must be designated upon DeviceBuffer creation (see [BufferDescription.StructureByteStride](xref:NeoVeldrid.BufferDescription#NeoVeldrid_BufferDescription_StructureByteStride)).

Structured buffers may be read-only or read-write. Read-write buffers can be written to in the fragment and compute stages, allowing arbitrary data to be output by shaders. Read-only structured buffers must be created with the [BufferUsage.StructuredBufferReadOnly](xref:NeoVeldrid.BufferUsage) flag, and read-write structured buffers must be created with the [BufferUsage.StructuredBufferReadWrite](xref:NeoVeldrid.BufferUsage) flag.
...
A [DeviceBufferRange](xref:NeoVeldrid.DeviceBufferRange) is a simple wrapper struct that describes a range of a [DeviceBuffer](xref:NeoVeldrid.DeviceBuffer). When included in a ResourceSet, a DeviceBufferRange makes only a subset of the DeviceBuffer available to be read from or written to by the shader. This is useful when you want to store multiple distinct blocks in a DeviceBuffer and use each in different draw calls, or if you want a compute shader invocation to only fill in a small portion of a buffer.

### TextureView

A [TextureView](xref:NeoVeldrid.TextureView) is a resource which gives a shader read-only or read-write access to a [Texture](xref:NeoVeldrid.Texture). A TextureView allows a subset of the Texture object's dimensions to be accessible, enabling a single slice of an array texture to be read from or written to, for example.

It is also possible to bind a [Texture](xref:NeoVeldrid.Texture) directly into a slot expecting a TextureView. Doing this is functionally equivalent to binding a TextureView that covers the Texture's full range of mip levels and array layers, with the same PixelFormat.

Read-only TextureView objects must have a [Target](xref:NeoVeldrid.TextureViewDescription#NeoVeldrid_TextureViewDescription_Target) that was created with the [TextureUsage.Sampled](xref:NeoVeldrid.TextureUsage) flag. Read-write TextureViews must target a Texture created with the [TextureUsage.Storage](xref:NeoVeldrid.TextureUsage) flag.

TextureViews can have a different [PixelFormat](xref:NeoVeldrid.PixelFormat) from the Texture they target, with some restrictions. This allows you to reinterpret Texture data between different storage types (UNorm, SNorm, UInt, SInt, Float). For views over uncompressed Textures, the overall size and number of components in the view's format must be equal to the underlying Texture's format. For views over compressed Textures, it is only possible to use the underlying Texture's exact PixelFormat or its sRGB/non-sRGB counterpart.
...
A [Sampler](xref:NeoVeldrid.Sampler) is a resource which controls how TextureViews are sampled. See [SamplerDescription](xref:NeoVeldrid.SamplerDescription) for the set of properties governing their behavior.

Several shared Samplers are available as properties on [GraphicsDevice](xref:NeoVeldrid.GraphicsDevice). These can be used when a common type of Sampler is needed and you don't want to manage the lifetime of a shared Sampler yourself.

Anisotropic filtering, while very common, is not supported on all [GraphicsDevice](xref:NeoVeldrid.GraphicsDevice) instances. [GraphicsDeviceFeatures.SamplerAnisotropy](xref:NeoVeldrid.GraphicsDeviceFeatures#NeoVeldrid_GraphicsDeviceFeatures_SamplerAnisotropy) indicates whether anisotropic filtering is supported.
...
* Vulkan/SPIR-V: ResourceLayouts match very closely with regular Vulkan and SPIR-V conventions. In a Vulkan shader, a uniform's "set" layout specifies the ResourceLayout index in the overall [ResourceLayouts](xref:NeoVeldrid.GraphicsPipelineDescription#NeoVeldrid_GraphicsPipelineDescription_ResourceLayouts) array. The "binding" layout specifies the specific resource within that ResourceLayout identified by the "set". For example:
```
layout(set = 0, binding = 1) uniform View
```
defines a uniform belonging to binding 1 (of the [ResourceLayoutElementDescription](xref:NeoVeldrid.ResourceLayoutDescription#NeoVeldrid_ResourceLayoutDescription_Elements) array), of set 0 (of the [ResourceLayouts](xref:NeoVeldrid.GraphicsPipelineDescription#NeoVeldrid_GraphicsPipelineDescription_ResourceLayouts) array).
...
* Direct3D 11: Resources are assigned HLSL registers based on their positions in the [ResourceLayouts](xref:NeoVeldrid.GraphicsPipelineDescription#NeoVeldrid_GraphicsPipelineDescription_ResourceLayouts) array first, and then by their position in the [ResourceLayoutElementDescriptions](xref:NeoVeldrid.ResourceLayoutDescription#NeoVeldrid_ResourceLayoutDescription_Elements) array. Each resource type (texture, sampler, uniform) is assigned an increasing integer value for its register number. For example, given these two ResourceLayouts:
