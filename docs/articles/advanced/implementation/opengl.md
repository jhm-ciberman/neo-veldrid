...
To create an OpenGL or OpenGL ES GraphicsDevice, several pieces of pre-configured "context" information are needed (see [OpenGLPlatformInfo](xref:NeoVeldrid.OpenGL.OpenGLPlatformInfo)). This information includes:

* The OpenGL context handle, created outside of NeoVeldrid.
...
These functions are platform-specific, and different operating systems vary in how they implement these functions. As such, it is difficult to write portable logic which satisfies all of these pieces of information. The NeoVeldrid.StartupUtilities library includes functionality to obtain this information portably, using SDL2.

## API Concept Map

| NeoVeldrid Concept | OpenGL Concept | Notes |
| --------------- | --------------| ----- |
| [GraphicsDevice](xref:NeoVeldrid.GraphicsDevice) | GL Context (roughly) | Various other parts of OpenGL support GraphicsDevice functionality. |
| [CommandList](xref:NeoVeldrid.CommandList) | Emulated (see below) |
| [DeviceBuffer](xref:NeoVeldrid.DeviceBuffer) | [Buffer objects](https://www.khronos.org/opengl/wiki/Buffer_Object) | |
| [BufferUsage](xref:NeoVeldrid.BufferUsage) | None | OpenGL buffers can always be used as any type of buffer -- there is no specialization. |
| [Texture](xref:NeoVeldrid.Texture) | [Texture objects](https://www.khronos.org/opengl/wiki/Texture) | |
| [TextureUsage](xref:NeoVeldrid.TextureUsage) | None | OpenGL textures can always be used as any type of texture -- there is no specialization. |
| [TextureView](xref:NeoVeldrid.TextureView) | [`ARB_texture_view`](https://www.khronos.org/opengl/wiki/Texture_Storage#Texture_views) | Not all NeoVeldrid TextureViews require `ARB_texture_view`. |
| [Sampler](xref:NeoVeldrid.Sampler) | [Sampler objects](https://www.khronos.org/opengl/wiki/Sampler_Object) | |
| [Pipeline](xref:NeoVeldrid.Pipeline) | Compound (see below) | |
| [Blend State](xref:NeoVeldrid.BlendStateDescription) | `glBlendColor`, `glBlendFuncSeparatei`, `glBlendEquationSeparatei` | |
| [Depth Stencil State](xref:NeoVeldrid.DepthStencilStateDescription) | `glDepthFunc`, `glDepthMask`, `glStencilFuncSeparate`, `glStencilMask` | |
| [Rasterizer State](xref:NeoVeldrid.RasterizerStateDescription) | `glCullFace`, `GL_CULL_FACE`, `glPolygonMode`, `GL_SCISSOR_TEST`, `glScissorIndexed`, `GL_DEPTH_CLAMP`, `glFrontFace` | |
| [PrimitiveTopology](xref:NeoVeldrid.PrimitiveTopology) | `GL_LINES`, `GL_POINTS`, `GL_TRIANGLES`, etc. | |
| [Vertex Layouts](xref:NeoVeldrid.VertexLayoutDescription) | `glEnableVertexAttribArray`, `glVertexAttrib*Pointer`, `glVertexAttribDivisor` | A single, global VAO is bound during initialization, and vertex attributes are updated into it as needed. |
| [Shader](xref:NeoVeldrid.Shader) | [Shader objects](https://www.khronos.org/opengl/wiki/Shader) | |
| [ShaderSetDescription](xref:NeoVeldrid.ShaderSetDescription) | [Program objects](https://www.khronos.org/opengl/wiki/GLSL_Object#Program_objects). | |
| [Framebuffer](xref:NeoVeldrid.Framebuffer) | [Framebuffer objects](https://www.khronos.org/opengl/wiki/Framebuffer_Object).aspx) | |
| [ResourceLayout](xref:NeoVeldrid.ResourceLayout) | None | The OpenGL backend uses the information from a ResourceLayout to determine which uniform locations a particular resource corresponds to, by name. |
| [ResourceSet](xref:NeoVeldrid.ResourceSet) | None | The OpenGL backend simply uses the individual resources contained in a ResourceSet to bind to a program. The slots are determined using information from the associated ResourceLayout. |
| [Swapchain](xref:NeoVeldrid.GraphicsDevice#NeoVeldrid_GraphicsDevice_SwapchainFramebuffer) | None (implicit) | An OpenGL swapchain is not explicitly created by the user, nor can it be directly accessed or managed. Instead, it is part of the OpenGL context created by the host system.

## Notes

The OpenGL backend is the most complicated backend, and has to work around various limitations in the OpenGL API to provide a full-fidelity implementation of NeoVeldrid's abstraction. Although it is a highly portable backend and can be used on many operating systems, it is not the preferred backend on any system, because of the following:

* OpenGL driver quality is not consistent. Performance and correctness varies wildly across vendors and operating systems. The NeoVeldrid OpenGL backend has a variety of code paths working around the presence and absence of certain extensions, and avoids using some features altogether due to their poor implementations on some systems.
...
* OpenGL's lack of support for multiple threads requires more complex and costly handling in NeoVeldrid's OpenGL backend, which imposes some minor runtime overhead. See below for more details.
